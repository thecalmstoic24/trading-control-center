...
def validate_config(value):
    ...
    timeframe=value.get('timeframe',1)
    exclusion=value.get('excludeAboveMedian',0)
    if not isinstance(reference,str) or (reference and not re.fullmatch(r'(?:NQ|MNQ) (?:MAR|JUN|SEP|DEC)\d{2}',reference)): raise ValueError('Select an NQ/MNQ reference contract.')
    if type(timeframe) is not int or timeframe not in (1,3,5,15): raise ValueError('Select a 1, 3, 5, or 15-minute timeframe.')
    if isinstance(exclusion,bool) or not isinstance(exclusion,(int,float)) or not math.isfinite(exclusion) or (exclusion != 0 and not 1<=exclusion<=20): raise ValueError('Median exclusion must be 0 (off) or 1–20 times the median.')
    return dict(enabled=value['enabled'],vm=vm,bars=bars,multiplier=multiplier,reference=reference,timeframe=timeframe,excludeAboveMedian=exclusion)
...
def size(spec,config,view,clock=None):
    ...
    if not isinstance(feed,dict) or feed.get('periodMinutes')!=config['timeframe']: raise ValueError('Enable TccTelemetry on a matching '+str(config['timeframe'])+'-minute NQ/MNQ chart on the source VM.')
    ...
    if stamp.tzinfo is None or not -5<=(clock-stamp).total_seconds()<=config['timeframe']*60+30: raise ValueError('Completed candle data is stale. Auto Quantity has not been applied.')
    ...
    reference=config['reference'].split(' ',1) if config['reference'] else actual
    if reference!=actual: raise ValueError('Source chart does not match the selected reference instrument and contract.')
    bars=feed.get('bars',[])
    ...
    if (clock-closed).total_seconds()>config['timeframe']*60+90: raise ValueError('Most recent completed candle is stale.')
    if config['excludeAboveMedian']:
        threshold=statistics.median(ranges)*config['excludeAboveMedian']
        ranges=[value for value in ranges if value<=threshold]
        if not ranges: raise ValueError('No completed candles remain after median exclusion.')
    ...
    return dict(ticker=root+' '+expected[1] if len(expected)>1 else root,leftQuantity=left,rightQuantity=right,averageRange=average,distance=distance,bars=config['bars'],usedBars=len(ranges),timeframe=config['timeframe'],reference=config['reference'] or feed['instrument'],source=config['vm'],publishedUtc=feed['publishedUtc'],estimatedLeftProfit=left*distance*(20 if root=='NQ' else 2))

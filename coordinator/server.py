...
class Handler(BaseHTTPRequestHandler):
    ...
    def reply(self, status, value, kind='application/json; charset=utf-8'):
        ...
        for key, val in {'Content-Type':kind, 'Content-Length':str(len(data)), 'Cache-Control':'no-store',
                         ...
                         'Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-src https://www.tradingview.com https://www.tradingview-widget.com; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"}.items():

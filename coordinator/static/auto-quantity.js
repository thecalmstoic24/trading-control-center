...
(() => {
 ...
 let config={enabled:false,vm:'',bars:10,multiplier:1,reference:'',timeframe:1,excludeAboveMedian:3},fleet=[];
 ...
 const bars=number(1,100,1,10),multiplier=number(.1,20,.1,1);
 const save=node('button','Save');save.type='button';
 const preview=node('button','Preview');preview.type='button';preview.className='quiet';
 ...
 const fields=node('div');fields.className='auto-fields';fields.append(label('Source VM',source),label('Reference',reference),label('Timeframe',timeframe),label('Lookback · bars',bars),label('Distance · ×',multiplier));
 const actions=node('div');actions.className='auto-actions';actions.append(save,preview);
 header.insertBefore(actions,enabledLabel);
 host.append(header,fields,referenceOptions,node('p','Anchor: left profit target · Auto NQ / MNQ · Preserve ratio. Recalculate before each trade; lock at Preparing.'),message);
 ...
 const settings=()=>({enabled:enable.checked,vm:source.value,bars:Number(bars.value),multiplier:Number(multiplier.value),reference:reference.value.trim().toUpperCase(),timeframe:Number(timeframe.value),excludeAboveMedian:3});
 ...
 enable.onchange=()=>{config={...config,enabled:enable.checked,excludeAboveMedian:3};document.body.classList.toggle('auto-enabled',enable.checked);window.dispatchEvent(new Event('auto-quantity-changed'));};
 ...
 api('/api/auto-quantity').then(value=>{config={...value,excludeAboveMedian:3};sources();apply();}).catch(e=>message.textContent=e.message);

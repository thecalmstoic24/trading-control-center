...
a.accounts=['Sim101','MFF-123','MFF-456'];a.matchedAccounts=['MFF-123'];a.matchStatus='checked';
vm.runInContext("selectView('a')",context);render(s);
const unmatched=get('left-account').children.find(o=>o.value==='MFF-456');
assert.equal(unmatched.disabled,false,'unmatched NinjaTrader account must remain selectable');
assert.match(unmatched.textContent,/not matched/);
a.matchStatus='unknown';a.matchedAccounts=[];render(s);
assert.match(get('left-account').children.find(o=>o.value==='MFF-456').textContent,/unknown/);
console.log('V15 dashboard: Airtable matching labels never filter available accounts.');

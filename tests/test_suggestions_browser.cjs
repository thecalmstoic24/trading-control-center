...
(async()=>{
 ...
 async function open(){
  ...
  await page.route('http://127.0.0.1:8788/**',async route=>{
   ...
   }else if(url.pathname==='/api/state')result={version:'16.0-preview.33',fleet:[],pairs:[],events:[],vmEvents:[],limits:{vms:50,pairs:20}};
 ...
 assert.match(await page.title(),/Preview 33/);
 assert.match(await page.locator('#control-center-title').textContent(),/Preview 33/);
 ...
 assert.equal(await page.locator('.draft-card').count(),0);assert.deepEqual(writes,[]);
 await context.close();
 // Preview 33: exact user examples, current website snapshot correction, and

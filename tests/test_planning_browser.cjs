...
(async()=>{
 ...
 const accounts=()=>page.locator('#planning-table tbody tr td:nth-child(3)').allTextContents();
 ...
 assert.deepEqual(await accounts(),['B','A']);
 await page.locator('#planning-column-options summary').click();
 await page.getByRole('checkbox',{name:'Balance',exact:true}).uncheck();
 assert.equal(await page.locator('#planning-table th').count(),3);
 ...
 assert.deepEqual(await page.locator('#planning-table thead th').allTextContents(),['Select','Balance','Pair status','id']);
 await page.reload();await page.locator('#tab-planning').click();
 await page.waitForFunction(()=>document.querySelectorAll('#planning-table tbody tr').length===3);
 assert.deepEqual(await page.locator('#planning-table thead th').allTextContents(),['Select','Balance','Pair status','id']);

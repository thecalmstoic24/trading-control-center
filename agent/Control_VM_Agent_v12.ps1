...
function Assert-V10Safety {
    ...
    if ($Snapshot.Account -cne $LockedAccount) {
        throw "BLOCKED: v10.4 only permits '$LockedAccount'. NinjaTrader shows '$($Snapshot.Account)'."
    }
    if ($RequireFlat -and -not (Test-NumberEquals -Actual $Snapshot.Quantity -Expected $LockedQuantity)) {
        throw "BLOCKED: v10.4 order quantity is locked to $LockedQuantity. NinjaTrader shows '$($Snapshot.Quantity)'."
    }
...
function Prepare-Trade {
    ...
    Set-UiaValue -Element $mainQuantity -Value '1' -Label 'Chart Trader order quantity'
    ...
    if (-not (Test-NumberEquals -Actual $readMainQuantity -Expected $LockedQuantity)) {
        throw "Chart Trader order quantity verification failed. Expected 1, found '$readMainQuantity'."
    }
    ...
    try {
        ...
        Set-UiaValue -Element $topQuantity -Value '1' -Label 'Order quantity'
        ...
        if (-not (Test-NumberEquals -Actual $readOrderQuantity -Expected $LockedQuantity)) {
            throw "Order quantity verification failed. Expected 1, found '$readOrderQuantity'."
        }
...
$lockNotice.Text = 'TEST LOCK: Account Sim101  |  Order quantity 1  |  Parameter type Currency'
        ...
        try {
            ...
            if ($reply.command -eq 'monitor_status') {
                ...
                $check.PeerFlat = $reply.ok -and $age -le $script:MaxStateAgeMs -and
                    ...
                    $reply.account -ceq $LockedAccount -and $reply.position -ceq 'Flat'
...
$prepareButton.Add_Click({
    ...
    try {
        ...
        if ($pairEnabled.Checked -and -not $script:RemoteCommandActive) {
            ...
            if ($peerResult.account -cne $LockedAccount -or [string]$peerResult.quantity -ne '1' -or $peerResult.position -cne 'Flat') {
        ...
        $script:EntryFault = $false
        $prepareStatus.Text = "READY - $($result.Ticker), Qty 1, Currency, SL $parsedStopLoss, Profit $parsedProfit.$peerPreparedMessage"
        ...
        if (-not $script:RemoteCommandActive) {
            ...
            [System.Windows.Forms.MessageBox]::Show(
                "PREPARED successfully.`n`nAccount: $LockedAccount`nTicker: $($result.Ticker)`nQuantity: 1`nParameter type: Currency`nLocal stop loss: $parsedStopLoss`nLocal profit: $parsedProfit$peerSummary`n`nNo trade was placed. Execution is now enabled.",
...
function Invoke-Entry {
    ...
    try {
        ...
        $executionStatus.Text = "Sending $Side Market to Sim101..."
        $form.Refresh()
...
function Invoke-Close {
    ...
    try {
        ...
        $executionStatus.Text = 'Closing Sim101 position...'
        $form.Refresh()
...
function Invoke-PairedEntry {
    ...
    if ($peerStatus.account -cne $LockedAccount -or [string]$peerStatus.quantity -ne '1') {
    ...
    if (-not $peerArm.ok -or -not $peerArm.scheduled) {
        ...
        Invoke-PairEmergencyClose -Reason "Entry handshake failed: $failure"
...
function Monitor-ActivePair {
    ...
    try {
        ...
        if ($peer.account -cne $LockedAccount) {
